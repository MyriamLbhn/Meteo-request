...
def on_marker_click(lat, lon):
    # Requête vers l'API OpenWeatherMap pour récupérer les données météorologiques de la ville
    weather_url = f'https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={API_KEY}&units=metric'
    weather_response = requests.get(weather_url)
    weather_data = weather_response.json()

    # Affichage des informations de la ville dans col1
    show_weather_data(weather_data['name'])
    st.session_state.city = weather_data['name']


with col1:
    # Affichage des informations de la ville sélectionnée ou cliquée
    if 'city' in st.session_state:
        show_weather_data(st.session_state.city)
